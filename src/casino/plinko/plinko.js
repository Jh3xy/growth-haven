...
const MULT_TABLES = {
  low: {
    ...
    16: [16.0,  9.0,  4.0,  2.0,  1.4, 1.1,  1.0,  0.7,  0.5,  0.7,  1.0,  1.1,  1.4,  2.0,  4.0,  9.0, 16.0],
  },
  medium: {
    8:  [13.0,  3.0,  1.3,  0.7,  0.4, 0.7,  1.3,  3.0, 13.0],
    12: [24.0,  5.0,  2.0,  0.9,  0.5, 0.3,  0.2,  0.3,  0.5,  0.9,  2.0,  5.0, 24.0],
    16: [110.0, 41.0, 10.0, 5.0,  2.0, 0.9,  0.5,  0.3,  0.2,  0.3,  0.5,  0.9,  2.0,  5.0, 10.0, 41.0, 110.0],
  },
  high: {
    8:  [29.0,  4.0,  1.5,  0.3,  0.2, 0.3,  1.5,  4.0, 29.0],
    12: [46.0,  6.0,  2.0,  0.5,  0.2, 0.1,  0.1,  0.1,  0.2,  0.5,  2.0,  6.0, 46.0],
    16: [999.0, 200.0, 60.0, 15.0, 3.0, 0.9,  0.3,  0.2,  0.1,  0.2,  0.3,  0.9,  3.0, 15.0, 60.0, 200.0, 999.0],
  },
}
...
// consistent visual height regardless of which mode is chosen.
function rowSpacing(rows) {
  return Math.max(26, Math.floor(400 / rows))
}

// Board wrap height: enough to contain all peg rows + landing zone.
function boardHeight(rows) {
  return PEG_MARGIN + rows * rowSpacing(rows) + 32
}
...
function boardUnit(rows) {
  const w = Math.max(boardWrap.clientWidth, 240)
  return w / (rows + 1)
}
...
function snapBallToStart() {
  ...
  boardEl.querySelectorAll('.plinko-peg.is-active')
    .forEach(el => el.classList.remove('is-active'))
  bucketsEl.querySelectorAll('.plinko-bucket.is-landed')
...
async function animateDrop(path, bucketIndex) {
  ...
  for (let r = 0; r < rows; r++) {
    ...
    const pegEl = boardEl.querySelector(`.plinko-peg[data-row="${r}"][data-peg="${col}"]`)
  ...
  const bucketEl = bucketsEl.querySelector(`.plinko-bucket[data-index="${bucketIndex}"]`)
...
function updateStats() {
  ...
  if (maxMultEl)    maxMultEl.textContent    = `${max}×`
  if (minMultEl)    minMultEl.textContent    = `${min}×`
}
...
function setDisabled(disabled) {
  ...
  betChips?.querySelectorAll('.dice-chip').forEach(b => b.disabled = disabled)
}
...
renderBoard()
updateStats()
const recentBets = initRecentBets('plinko', document.getElementById('recentBetsMount'))
await loadWallet()
